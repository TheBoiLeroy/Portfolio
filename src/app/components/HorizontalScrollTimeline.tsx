...
export default function TimelineSteps() {
  ...
  // ✅ Subscribe to scroll progress changes in an effect (avoids resubscribing on every render)
  useEffect(() => {
    const unsub = scrollYProgress.on("change", (p) => {
      const idx = Math.floor(p * timelineData.length);
      setActiveIndex(idx >= timelineData.length ? timelineData.length - 1 : idx);
    });
    return () => unsub();
  }, [scrollYProgress]);
  ...
  const collectedTools: string[] = useMemo(() => {
    ...
    for (let i = 0; i <= activeIndex; i++) {
      const t = timelineData[i]?.tools;
      if (t) t.split(/,\s*/).forEach((tool) => toolsSet.add(tool));
    }
    return Array.from(toolsSet);
  }, [activeIndex]);

  // Precompute final screen transforms at top level (not inside JSX props)
  const finalOpacity = useTransform(scrollYProgress, [0.98, 1], [0, 1]);
  const finalScale = useTransform(scrollYProgress, [0.98, 1], [0.95, 1]);
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {/* ✅ Final card transforms computed at top level (not inline hooks) */}
      <motion.div
...
/** Child component so hooks aren’t inside array callbacks */
function TimelineStep({
  ...
  // ✅ Hooks at top level of a component (safe)
